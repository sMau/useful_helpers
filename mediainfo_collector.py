...
if __name__ == "__main__":
	...
	for f in files:
		info = MediaInfo(filename=f, cmd='mediainfo')
		...
		try:
			...
			if 'HEVC' in i['videoCodec']:
				hevc.append(f)
			if float(i['videoWidth']) > 1920:
				fourk.append(f)
			if float(i['audioChannel']) < 6:
				no_sourround.append(f)
			if float(i['bitrate']) < 5000000:
				low_bitrate.append(f)
			if float(i['bitrate']) > 15000000:
				high_bitrate.append(f)
		except KeyError:
			info_missing.append(f)
